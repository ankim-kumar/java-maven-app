pipeline {
    ...
    stages {
        ...
        stage('Clone the git repo') {
            steps {
                git branch: 'Dev', url: 'https://github.com/ankim-kumar/java-maven-app.git'
            }
        }
        ...
        stage('Docker images/container remove') {
            steps {
                sh ''' docker stop javaBpro_container_dev || true
                       docker rm javaBpro_container_dev || true
                       docker rmi ankim628/javabpro-dev:1  || true '''
            ...
            steps {
                script{
                    withDockerRegistry(credentialsId: 'docker-id', toolname: 'docker'){
                        sh '''docker build -t ankim628/javabpro-dev:1 .
                              docker push ankim628/javabpro-dev:1'''
                    }
                }    
            }
        }
        stage('Docker container of web app ') {
            steps {
                sh 'docker run -d -p 9002:8080 --name javaBpro_container_dev -t ankim628/javabpro-dev:1'
            }
        }
    }    
        post {
